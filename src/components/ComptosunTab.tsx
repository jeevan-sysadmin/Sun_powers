...
const ComptosunTab: React.FC<ComptosunTabProps> = ({
  batteries,
  filteredBatteries,
  filterStatus,
  filterBatteryType,
  ...
  // Filter batteries to only show "comptosun" claim_type
  const comptosunBatteries = batteries.filter(battery => 
    battery.claim_type && battery.claim_type.toLowerCase() === 'comptosun'
  );
  
  const filteredComptosunBatteries = filteredBatteries.filter(battery => 
    battery.claim_type && battery.claim_type.toLowerCase() === 'comptosun'
  );
  ...
  useEffect(() => {
    let filtered = filteredComptosunBatteries.filter(battery => {
      // Search filter
      if (localSearchTerm) {
  ...
  }, [filteredComptosunBatteries, localSearchTerm, filterStatus, filterBatteryType, dateFilterType, customDateRange, selectedYear, selectedMonth]);
  ...
  const getStatusCount = (status: string) => {
    if (status === 'active') {
      return comptosunBatteries.filter(b => !b.status || b.status === '' || b.status === 'active').length;
    }
    return comptosunBatteries.filter(b => b.status === status).length;
  };
  ...
  const exportToCSV = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'CompanyToSun Batteries');
      XLSX.writeFile(wb, `comptosun_batteries_export_${new Date().toISOString().split('T')[0]}.csv`);
    } catch (error) {
  ...
  const exportToPDF = () => {
    try {
      ...
      doc.text('SUN POWERS - COMPANY TO SUN BATTERIES REPORT', 148.5, 13, { align: 'center' });
      ...
      autoTable(doc, {
        ...
        didDrawPage: (data) => {
          ...
          for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text(
              `Page ${i} of ${pageCount} - Sun Powers Company to Sun Report - ${getDateRangeText()}`,
      ...
      doc.save(`comptosun_report_${new Date().toISOString().split('T')[0]}.pdf`);
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Sun Powers - Company to Sun Batteries Report</title>
          ...
          <body>
            <div class="print-container">
              <h1>🔋 Sun Powers - Company to Sun Batteries Report</h1>
  ...
  const batteryTypes = [...new Set(comptosunBatteries
  ...
  return (
    <div className="comptosun-section">
      {/* Mobile Header */}
      {windowWidth < 768 && (
        <div className="mobile-header">
          <div className="mobile-header-left">
            ...
            <h3>Company to Sun Batteries</h3>
          </div>
      ...
      {windowWidth >= 768 && (
        <div className="section-header">
          <div className="section-title">
            <h2>Company to Sun Powers Batteries</h2>
            <p>Batteries claimed from Company to Sun Powers - Showing {filteredData.length} of {comptosunBatteries.length} batteries</p>
          </div>
        </div>
      )}
      ...
      <div className="stats-cards">
        <div className="stat-card">
          ...
          <div className="stat-content">
            <h3>{comptosunBatteries.length}</h3>
            <p>Total Batteries</p>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            <h3>{comptosunBatteries.filter(b => b.warranty_period && b.warranty_period !== '0').length}</h3>
            <p>Under Warranty</p>
          </div>
        </div>
      </div>
        ...
        <div className="table-container" ref={tableContainerRef}>
          {loading ? (
            <div className="loading-state">
              ...
              <p>Loading Company to Sun batteries...</p>
            </div>
                  ...
                  <tbody>
                    {currentItems.map((battery, index) => (
                      ...
                      >
                        ...
                        <td>
                          <span className="delivery-code" style={{ color: '#3b82f6' }}>{battery.battery_code}</span>
                        </td>
                        <td>
                          <div className="product-cell">
                            <FiBattery className="product-icon" style={{ color: '#3b82f6' }} />
            ...
            <div className="empty-state">
              ...
              <h3>No Company to Sun batteries found</h3>
      ...
      {!loading && filteredData.length > 0 && (
        <div className="summary-footer">
          <div className="summary-info">
            <span className="claim-type-badge" style={{ backgroundColor: getClaimColor('comptosun') + '20', color: getClaimColor('comptosun') }}>
              <FiArrowLeft style={{ marginRight: '4px' }} /> COMPANY TO SUN CLAIM
            </span>
            <span className="divider">•</span>
            <span>
              Showing {filteredData.length} of {comptosunBatteries.length} batteries
            </span>
